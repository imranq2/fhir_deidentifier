...
validate:
	...
	docker compose -f docker-compose-validate.yml exec validate-script sh -c "pip install --root-user-action=ignore --upgrade pip && pip install --root-user-action=ignore requests && python download_profiles.py && python upload_profiles.py && python validate.py /data/output/large"
	docker compose -f docker-compose-validate.yml down
